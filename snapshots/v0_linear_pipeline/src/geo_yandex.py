...
def fill_geo_yandex_file(
    ...
    # Чтение секретов из папки secrets
    ...
    api_key = (
        os.getenv("YANDEX_GEOCODER_KEY") or
        read_secret_file("yandex_key.txt") or
        cfg.get("api", "yandex_geocoder_key", fallback="")
    ).strip()
    ...
    missing_geo_path = Path(cfg.get("paths", "missing_geo", fallback="logs/missing_geo.csv"))
