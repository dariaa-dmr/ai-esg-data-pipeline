...
from row_identity import make_record_id, normalize_inn
...
def _reconstruct_tail_after_inn(before_inn: list[str], inn: str, after_inn: list[str], delimiter: str) -> list[str]:
    ...
    if len(before_inn) == 5:
        ...
        # If there are fewer fields before INN, the safest assumption for old/short
        # exports is that the available values are the company short and official
        # names immediately preceding INN. Pad missing classification fields on the
        # LEFT, not on the right, so Sector/Industry/Subindustry stay empty instead
        # of being filled with company names and misrouting the record.
        prefix = [""] * (5 - len(before_inn)) + before_inn
...
def repair_fields(fields: list[str], delimiter: str = ",") -> tuple[Optional[list[str]], str]:
    ...
    if inn_idx is not None:
        before_inn = fields[:inn_idx]
        after_inn = fields[inn_idx + 1 :]
        repaired = _reconstruct_tail_after_inn(before_inn, inn_value, after_inn, delimiter)
        if len(repaired) == EXPECTED_NCOLS:
            review_suffix = "_needs_review" if (len(fields) != EXPECTED_NCOLS or len(before_inn) < 5 or len(after_inn) < 6) else ""
            return repaired, f"repaired_by_inn_anchor{review_suffix}_original_cols={len(fields)}"
...
def split_clean_dirty(
    ...
    Clean file receives EXPECTED_COLUMNS plus metadata columns.
    Dirty file receives EXPECTED_COLUMNS plus raw_line and diagnostic columns.
    """
    ...
    for record_no, physical_line, raw_record in iter_logical_records(input_path, encoding, delimiter):
        ...
        if parsed is not None:
            base = {col: _clean_field(parsed[i]) for i, col in enumerate(EXPECTED_COLUMNS)}
            parse_status = "clean_exact" if "parsed_exactly" in reason else "clean_repaired"
            if "padded_missing_fields" in reason or "needs_review" in reason:
                parse_status = "clean_repaired_needs_review"
            record_id = make_record_id(
                ...
                base.get("Sector"),
                base.get("Industry"),
                base.get("Subindustry"),
            )
            base.update({
                "record_id": record_id,
                "source_file": input_path.name,
                "source_row_no": str(record_no),
                "physical_line": str(physical_line),
    ...
    clean_df = pd.DataFrame(clean_rows, columns=EXPECTED_COLUMNS + CLEAN_METADATA_COLUMNS)
    dirty_df = pd.DataFrame(dirty_rows, columns=EXPECTED_COLUMNS + ["record_id", "raw_line", "dirty_reason", "source_file", "source_row_no", "physical_line"])
