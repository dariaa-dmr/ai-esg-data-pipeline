...
from integrity_check import check_and_restore, validate_row_count  # fallback-механизм
from paths import read_config, load_paths
from safe_filename import safe_stem_from_path
...
def setup_logging(paths: dict[str, Path]) -> logging.Logger:
    ...
    fh = logging.FileHandler(log_path, encoding="utf-8", errors="backslashreplace")
    fh.setFormatter(fmt)
...
def safe_stem(path: Path) -> str:
    return safe_stem_from_path(path)
...
def process_one_file(input_file: Path, cfg: configparser.ConfigParser, paths: dict[str, Path], logger: logging.Logger, cfg_path: Path) -> None:
    ...
    check_and_restore(enriched_path, "enriched", logger, clean_dir=paths["clean"], reports_dir=paths.get("reports"))
    validate_row_count(clean_path, enriched_path, "clean_to_enriched", logger)
    ...
    check_and_restore(geo_path, "geo", logger, clean_dir=paths["clean"], reports_dir=paths.get("reports"))
    validate_row_count(enriched_path, geo_path, "enriched_to_geo", logger)
...
def main() -> int:
    ...
    paths = load_paths(cfg, create=True).as_dict()
